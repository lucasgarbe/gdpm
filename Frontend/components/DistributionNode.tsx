        ...
        <div className="flex">
            {selected && (
                <div className="absolute -top-10 flex gap-2 rounded bg-gray-50 border border-gray-100">
                    ...
                    {showModal && (
                        <div className="overflow-auto p-1 absolute top-24 w-64 h-96 border border-blue-600 bg-blue-200"
                             onClick={() => setShowModal(false)}>
                            <div className="pb-2 overflow-auto">{data.dist.distType}</div>
                            <div className="overflow-auto p-1"><Link href={data.dist.url}
                                                                     target="_blank">{data.dist.url}</Link></div>
                            <ul>
                                {data.dist.input.map((input, index) => (
                                    <div key={index}>
                                        <li className="p-1"><p>id:</p> {input.id}</li>
                                        <li className="p-1"><p>name:</p> {input.name}</li>
                                        <li className="p-1"><p>type:</p> {input.type}</li>
                                        <li className="p-1"><p>upper:</p> {input.upper}</li>
                                        <li className="p-1"><p>lower:</p> {input.lower}</li>
